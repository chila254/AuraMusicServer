...
func (s *Server) handleMessage(c *Client, data []byte) {
	// Simple JSON decoding for now (no protobuf complexity)
	var msg struct {
	...
	case MsgTypeTransferHost:
		s.handleTransferHost(c, msg.Payload)
	case MsgTypePing:
...
func (c *Client) sendMessage(logger *zap.Logger, msgType string, payload interface{}) {
	// Encode using JSON for simplicity (no protobuf needed for now)
	data, err := json.Marshal(map[string]interface{}{
		"type":    msgType,
		"payload": payload,
	})
	if err != nil {
		logger.Error("Error encoding message", zap.String("message_type", msgType), zap.Error(err))
		return
	}
	...
	select {
	case c.Send <- data:
		logger.Debug("Message queued for sending", zap.String("message_type", msgType), zap.Int("size", len(data)))
	default:
